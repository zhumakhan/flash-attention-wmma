...
__global__ void flash10_async_q_inregs_16x64x128(half* Q, half* K, half* V, half* O, int s){
...
__global__ void flash10_async_q_inregs_48x64x128(half* Q, half* K, half* V, half* O, int s){
    ...
    wmma::fragment<wmma::accumulator, 16, 16, 16, half> qk_frag[24];
    ...
    for(int o = 0; o < s; o += 128){
        ...
        for(int i = 0; i < 8; i++){
            ...
            wmma::fill_fragment(qk_frag[i+16], 0.f);
            ...
            for(int j = 0; j < 4; j++){
                ...
                wmma::mma_sync(qk_frag[i], q_frag[j], k_frag, qk_frag[i]);
                wmma::mma_sync(qk_frag[i+8], q_frag[j+4], k_frag, qk_frag[i+8]);
                wmma::mma_sync(qk_frag[i+16], q_frag[j+8], k_frag, qk_frag[i+16]);
            }
        }
        ...
        for(int i = 0; i < 8; i++){
            ...
            c_max.x = max(c_max.x, qk_frag[i+16].x[0]);
            c_max.x = max(c_max.x, qk_frag[i+16].x[1]);
            c_max.y = max(c_max.y, qk_frag[i+16].x[2]);
            c_max.y = max(c_max.y, qk_frag[i+16].x[3]);
            c_max.x = max(c_max.x, qk_frag[i+16].x[4]);
            c_max.x = max(c_max.x, qk_frag[i+16].x[5]);
            c_max.y = max(c_max.y, qk_frag[i+16].x[6]);
            c_max.y = max(c_max.y, qk_frag[i+16].x[7]);
        }
        ...
        for(int i = 0; i < 8; i++){
            ...
            #pragma unroll
            for(int j = 0; j < 5; j += 4){
                qk_exp_frag[2].x[j] = qk_exp_frag[2].x[j+8] = exp2f(__half2float(qk_frag[i+16].x[j] - b_max.x));
                qk_exp_frag[2].x[j+1] = qk_exp_frag[2].x[j+9] = exp2f(__half2float(qk_frag[i+16].x[j+1] - b_max.x));
                qk_exp_frag[2].x[j+2] = qk_exp_frag[2].x[j+10] = exp2f(__half2float(qk_frag[i+16].x[j+2] - b_max.y));
                qk_exp_frag[2].x[j+3] = qk_exp_frag[2].x[j+11] = exp2f(__half2float(qk_frag[i+16].x[j+3] - b_max.y));
            }
...
__global__ void flash10_async_q_inregs_64x64x128(half* Q, half* K, half* V, half* O, int s){
    ...
    wmma::fragment<wmma::accumulator, 16, 16, 16, half> qk_frag[32];
    ...
    for(int o = 0; o < s; o += 128){
        ...
        for(int i = 0; i < 8; i++){
            wmma::fill_fragment(qk_frag[i], 0.0f);
            wmma::fill_fragment(qk_frag[i+8], 0.0f);
            wmma::fill_fragment(qk_frag[i+16], 0.0f);
            wmma::fill_fragment(qk_frag[i+24], 0.0f);
            ...
            for(int j = 0; j < 4; j++){
                ...
                wmma::mma_sync(qk_frag[i+8], q_frag[j+4], k_frag, qk_frag[i+8]);
                wmma::mma_sync(qk_frag[i+16], q_frag[j+8], k_frag, qk_frag[i+16]);
                wmma::mma_sync(qk_frag[i+24], q_frag[j+12], k_frag, qk_frag[i+24]);
            }
        }
        ...
        for(int i = 0; i < 8; i++){
            ...
            d_max.x = max(d_max.x, qk_frag[i+24].x[0]);
            d_max.x = max(d_max.x, qk_frag[i+24].x[1]);
            d_max.y = max(d_max.y, qk_frag[i+24].x[2]);
            d_max.y = max(d_max.y, qk_frag[i+24].x[3]);
            d_max.x = max(d_max.x, qk_frag[i+24].x[4]);
            d_max.x = max(d_max.x, qk_frag[i+24].x[5]);
            d_max.y = max(d_max.y, qk_frag[i+24].x[6]);
            d_max.y = max(d_max.y, qk_frag[i+24].x[7]);
        }
        ...
        for(int i = 0; i < 8; i++){
            ...
            #pragma unroll
            for(int j = 0; j < 5; j += 4){
                qk_exp_frag[3].x[j] = qk_exp_frag[3].x[j+8] = exp2f(__half2float(qk_frag[i+24].x[j] - b_max.x));
                qk_exp_frag[3].x[j+1] = qk_exp_frag[3].x[j+9] = exp2f(__half2float(qk_frag[i+24].x[j+1] - b_max.x));
                qk_exp_frag[3].x[j+2] = qk_exp_frag[3].x[j+10] = exp2f(__half2float(qk_frag[i+24].x[j+2] - b_max.y));
                qk_exp_frag[3].x[j+3] = qk_exp_frag[3].x[j+11] = exp2f(__half2float(qk_frag[i+24].x[j+3] - b_max.y));
            }
...
int main(){
    ...
    flash10_async_q_inregs_16x64x128<<<blocks64, 128>>>(dev_Q, dev_K, dev_V, dev_O, s);
    
    flash10_async_q_inregs_32x64x128<<<blocks128, 128>>>(dev_Q, dev_K, dev_V, dev_O, s);

    // flash10_async_q_inregs_48x64x128<<<blocks192, 128>>>(dev_Q, dev_K, dev_V, dev_O, s);

    flash10_async_q_inregs_64x64x128<<<blocks256, 128>>>(dev_Q, dev_K, dev_V, dev_O, s);
