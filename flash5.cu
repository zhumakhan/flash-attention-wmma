...
__global__ void flash10_async_q_inregs_32x64x128(half* Q, half* K, half* V, half* O, int s){
    ...
    __shared__ half k_sh[128*72];
    __shared__ half v_sh[128*72];
    ...
    for(int o = 0; o < s; o += 128){
        ...
        for(int i = 0; i < 8; i ++){
            ...
            for(int j = 0; j < 4; j++){
                wmma::load_matrix_sync(k_frag, k_sh + i*16*72 + j*16, 72);
                wmma::mma_sync(qk_frag[i], q_frag[j], k_frag, qk_frag[i]);
                wmma::mma_sync(qk_frag[i+8], q_frag[j+4], k_frag, qk_frag[i+8]);
            }
        }
        __syncwarp();
        #pragma unroll
        for(int i = 0; i < 8; i++){
            ...
            b_max.x = max(b_max.x, qk_frag[i+8].x[0]);
            b_max.x = max(b_max.x, qk_frag[i+8].x[1]);
            b_max.y = max(b_max.y, qk_frag[i+8].x[2]);
            b_max.y = max(b_max.y, qk_frag[i+8].x[3]);
            b_max.x = max(b_max.x, qk_frag[i+8].x[4]);
            b_max.x = max(b_max.x, qk_frag[i+8].x[5]);
            b_max.y = max(b_max.y, qk_frag[i+8].x[6]);
            b_max.y = max(b_max.y, qk_frag[i+8].x[7]);
        }
        ...
        for(int i = 0; i < 8; i++){
            ...
            #pragma unroll
            for(int j = 0; j < 5; j += 4){
                qk_exp_frag[1].x[j] = qk_exp_frag[1].x[j+8] = exp2f(__half2float(qk_frag[i+8].x[j] - b_max.x));
                qk_exp_frag[1].x[j+1] = qk_exp_frag[1].x[j+9] = exp2f(__half2float(qk_frag[i+8].x[j+1] - b_max.x));
                qk_exp_frag[1].x[j+2] = qk_exp_frag[1].x[j+10] = exp2f(__half2float(qk_frag[i+8].x[j+2] - b_max.y));
                qk_exp_frag[1].x[j+3] = qk_exp_frag[1].x[j+11] = exp2f(__half2float(qk_frag[i+8].x[j+3] - b_max.y));
            }
...
int main(){
    ...
    // flash10_async_q_inregs_32x64x128<<<blocks128, 128>>>(dev_Q, dev_K, dev_V, dev_O, s);
    
    // flash10_async_q_inregs_16x16x16<<<1, 32>>>(dev_Q, dev_K, dev_V, dev_O, 16);
