...
__global__ void flash10_async_q_inregs_16x64x32(half* Q, half* K, half* V, half* O, int s){
    ...
    __shared__ half k_sh[32*72];
    __shared__ half v_sh[32*72];
    ...
    wmma::fragment<wmma::accumulator, 16, 16, 16, half> qk_frag[2];
...
__global__ void flash10_async_q_inregs_32x64x32(half* Q, half* K, half* V, half* O, const int s){
    ...
    wmma::fragment<wmma::accumulator, 16, 16, 16, half> qk_frag[4];
    ...
    for(int o = 0; o < s; o += 32){
        ...
        for(int i = 0; i < 2; i ++){
            wmma::fill_fragment(qk_frag[i], 0.0f);
            wmma::fill_fragment(qk_frag[i+2], 0.0f);
            #pragma unroll
            for(int j = 0; j < 4; j++){
                wmma::load_matrix_sync(k_frag, k_sh + i*16*72 + j*16, 72);
                wmma::mma_sync(qk_frag[i], q_frag[j], k_frag, qk_frag[i]);
                wmma::mma_sync(qk_frag[i+2], q_frag[j+4], k_frag, qk_frag[i+2]);
            }
        }
        __syncwarp();
        #pragma unroll
        for(int i = 0; i < 2; i++){
            ...
            b_max.x = max(b_max.x, qk_frag[i+2].x[0]);
            b_max.x = max(b_max.x, qk_frag[i+2].x[1]);
            b_max.y = max(b_max.y, qk_frag[i+2].x[2]);
            b_max.y = max(b_max.y, qk_frag[i+2].x[3]);
            b_max.x = max(b_max.x, qk_frag[i+2].x[4]);
            b_max.x = max(b_max.x, qk_frag[i+2].x[5]);
            b_max.y = max(b_max.y, qk_frag[i+2].x[6]);
            b_max.y = max(b_max.y, qk_frag[i+2].x[7]);
        }
        ...
        for(int i = 0; i < 2; i++){
            ...
            #pragma unroll
            for(int j = 0; j < 5; j += 4){
                qk_exp_frag[1].x[j] = qk_exp_frag[1].x[j+8] = exp2f(__half2float(qk_frag[i+2].x[j] - b_max.x));
                qk_exp_frag[1].x[j+1] = qk_exp_frag[1].x[j+9] = exp2f(__half2float(qk_frag[i+2].x[j+1] - b_max.x));
                qk_exp_frag[1].x[j+2] = qk_exp_frag[1].x[j+10] = exp2f(__half2float(qk_frag[i+2].x[j+2] - b_max.y));
                qk_exp_frag[1].x[j+3] = qk_exp_frag[1].x[j+11] = exp2f(__half2float(qk_frag[i+2].x[j+3] - b_max.y));
            }
...
__global__ void flash10_async_q_inregs_48x64x32(half* Q, half* K, half* V, half* O, int s){
    ...
    wmma::fragment<wmma::accumulator, 16, 16, 16, half> qk_frag[6];
    ...
    for(int o = 0; o < s; o += 32){
        ...
        for(int i = 0; i < 2; i++){
            ...
            wmma::fill_fragment(qk_frag[i+2], 0.f);
            wmma::fill_fragment(qk_frag[i+4], 0.f);
            ...
            for(int j = 0; j < 4; j++){
                ...
                wmma::mma_sync(qk_frag[i], q_frag[j], k_frag, qk_frag[i]);
                wmma::mma_sync(qk_frag[i+2], q_frag[j+4], k_frag, qk_frag[i+2]);
                wmma::mma_sync(qk_frag[i+4], q_frag[j+8], k_frag, qk_frag[i+4]);
            }
        }
        ...
        for(int i = 0; i < 2; i++){
            ...
            c_max.x = max(c_max.x, qk_frag[i+4].x[0]);
            c_max.x = max(c_max.x, qk_frag[i+4].x[1]);
            c_max.y = max(c_max.y, qk_frag[i+4].x[2]);
            c_max.y = max(c_max.y, qk_frag[i+4].x[3]);
            c_max.x = max(c_max.x, qk_frag[i+4].x[4]);
            c_max.x = max(c_max.x, qk_frag[i+4].x[5]);
            c_max.y = max(c_max.y, qk_frag[i+4].x[6]);
            c_max.y = max(c_max.y, qk_frag[i+4].x[7]);
        }
        ...
        for(int i = 0; i < 2; i++){
            ...
            #pragma unroll
            for(int j = 0; j < 5; j += 4){
                qk_exp_frag[2].x[j] = qk_exp_frag[2].x[j+8] = exp2f(__half2float(qk_frag[i+4].x[j] - b_max.x));
                qk_exp_frag[2].x[j+1] = qk_exp_frag[2].x[j+9] = exp2f(__half2float(qk_frag[i+4].x[j+1] - b_max.x));
                qk_exp_frag[2].x[j+2] = qk_exp_frag[2].x[j+10] = exp2f(__half2float(qk_frag[i+4].x[j+2] - b_max.y));
                qk_exp_frag[2].x[j+3] = qk_exp_frag[2].x[j+11] = exp2f(__half2float(qk_frag[i+4].x[j+3] - b_max.y));
            }
...
__global__ void flash10_async_q_inregs_64x64x32(half* Q, half* K, half* V, half* O, int s){
    ...
    wmma::fragment<wmma::accumulator, 16, 16, 16, half> qk_frag[8];
    ...
    for(int o = 0; o < s; o += 32){
        ...
        for(int i = 0; i < 2; i++){
            wmma::fill_fragment(qk_frag[i], 0.0f);
            wmma::fill_fragment(qk_frag[i+2], 0.0f);
            wmma::fill_fragment(qk_frag[i+4], 0.0f);
            wmma::fill_fragment(qk_frag[i+6], 0.0f);
            ...
            for(int j = 0; j < 4; j++){
                ...
                wmma::mma_sync(qk_frag[i+2], q_frag[j+4], k_frag, qk_frag[i+2]);
                wmma::mma_sync(qk_frag[i+4], q_frag[j+8], k_frag, qk_frag[i+4]);
                wmma::mma_sync(qk_frag[i+6], q_frag[j+12], k_frag, qk_frag[i+6]);
            }
        }
        ...
        for(int i = 0; i < 2; i++){
            ...
            d_max.x = max(d_max.x, qk_frag[i+6].x[0]);
            d_max.x = max(d_max.x, qk_frag[i+6].x[1]);
            d_max.y = max(d_max.y, qk_frag[i+6].x[2]);
            d_max.y = max(d_max.y, qk_frag[i+6].x[3]);
            d_max.x = max(d_max.x, qk_frag[i+6].x[4]);
            d_max.x = max(d_max.x, qk_frag[i+6].x[5]);
            d_max.y = max(d_max.y, qk_frag[i+6].x[6]);
            d_max.y = max(d_max.y, qk_frag[i+6].x[7]);
        }
        ...
        for(int i = 0; i < 2; i++){
            ...
            #pragma unroll
            for(int j = 0; j < 5; j += 4){
                qk_exp_frag[3].x[j] = qk_exp_frag[3].x[j+8] = exp2f(__half2float(qk_frag[i+6].x[j] - b_max.x));
                qk_exp_frag[3].x[j+1] = qk_exp_frag[3].x[j+9] = exp2f(__half2float(qk_frag[i+6].x[j+1] - b_max.x));
                qk_exp_frag[3].x[j+2] = qk_exp_frag[3].x[j+10] = exp2f(__half2float(qk_frag[i+6].x[j+2] - b_max.y));
                qk_exp_frag[3].x[j+3] = qk_exp_frag[3].x[j+11] = exp2f(__half2float(qk_frag[i+6].x[j+3] - b_max.y));
            }
...
__global__ void flash10_async_q_inregs_64x64x16(half* Q, half* K, half* V, half* O, int s){
    ...
    __shared__ half k_sh[16*72];
    __shared__ half v_sh[16*72];
    ...
    for(int o = 0; o < s; o += 16){
        ...
        wmma::fill_fragment(qk_frag[0], 0.0f);
        wmma::fill_fragment(qk_frag[1], 0.0f);
        wmma::fill_fragment(qk_frag[2], 0.0f);
        wmma::fill_fragment(qk_frag[3], 0.0f);

        #pragma unroll
        for(int j = 0; j < 4; j++){
            wmma::load_matrix_sync(k_frag, k_sh + j*16, 72);
            wmma::mma_sync(qk_frag[0], q_frag[j], k_frag, qk_frag[0]);
            wmma::mma_sync(qk_frag[1], q_frag[j+4], k_frag, qk_frag[1]);
            wmma::mma_sync(qk_frag[2], q_frag[j+8], k_frag, qk_frag[2]);
            wmma::mma_sync(qk_frag[3], q_frag[j+12], k_frag, qk_frag[3]);
        }
    
        a_max.x = max(a_max.x, qk_frag[0].x[0]);
        a_max.x = max(a_max.x, qk_frag[0].x[1]);
        a_max.y = max(a_max.y, qk_frag[0].x[2]);
        a_max.y = max(a_max.y, qk_frag[0].x[3]);
        a_max.x = max(a_max.x, qk_frag[0].x[4]);
        a_max.x = max(a_max.x, qk_frag[0].x[5]);
        a_max.y = max(a_max.y, qk_frag[0].x[6]);
        a_max.y = max(a_max.y, qk_frag[0].x[7]);

        b_max.x = max(b_max.x, qk_frag[1].x[0]);
        b_max.x = max(b_max.x, qk_frag[1].x[1]);
        b_max.y = max(b_max.y, qk_frag[1].x[2]);
        b_max.y = max(b_max.y, qk_frag[1].x[3]);
        b_max.x = max(b_max.x, qk_frag[1].x[4]);
        b_max.x = max(b_max.x, qk_frag[1].x[5]);
        b_max.y = max(b_max.y, qk_frag[1].x[6]);
        b_max.y = max(b_max.y, qk_frag[1].x[7]);

        c_max.x = max(c_max.x, qk_frag[2].x[0]);
        c_max.x = max(c_max.x, qk_frag[2].x[1]);
        c_max.y = max(c_max.y, qk_frag[2].x[2]);
        c_max.y = max(c_max.y, qk_frag[2].x[3]);
        c_max.x = max(c_max.x, qk_frag[2].x[4]);
        c_max.x = max(c_max.x, qk_frag[2].x[5]);
        c_max.y = max(c_max.y, qk_frag[2].x[6]);
        c_max.y = max(c_max.y, qk_frag[2].x[7]);

        d_max.x = max(d_max.x, qk_frag[3].x[0]);
        d_max.x = max(d_max.x, qk_frag[3].x[1]);
        d_max.y = max(d_max.y, qk_frag[3].x[2]);
        d_max.y = max(d_max.y, qk_frag[3].x[3]);
        d_max.x = max(d_max.x, qk_frag[3].x[4]);
        d_max.x = max(d_max.x, qk_frag[3].x[5]);
        d_max.y = max(d_max.y, qk_frag[3].x[6]);
        d_max.y = max(d_max.y, qk_frag[3].x[7]);
        ...
        #pragma unroll
        for(int j = 0; j < 5; j += 4){
            qk_exp_frag[0].x[j] = qk_exp_frag[0].x[j+8] = exp2f(__half2float(qk_frag[0].x[j] - a_max.x));
            qk_exp_frag[0].x[j+1] = qk_exp_frag[0].x[j+9] = exp2f(__half2float(qk_frag[0].x[j+1] - a_max.x));
            qk_exp_frag[0].x[j+2] = qk_exp_frag[0].x[j+10] = exp2f(__half2float(qk_frag[0].x[j+2] - a_max.y));
            qk_exp_frag[0].x[j+3] = qk_exp_frag[0].x[j+11] = exp2f(__half2float(qk_frag[0].x[j+3] - a_max.y));
        }

        #pragma unroll
        for(int j = 0; j < 5; j += 4){
            qk_exp_frag[1].x[j] = qk_exp_frag[1].x[j+8] = exp2f(__half2float(qk_frag[1].x[j] - b_max.x));
            qk_exp_frag[1].x[j+1] = qk_exp_frag[1].x[j+9] = exp2f(__half2float(qk_frag[1].x[j+1] - b_max.x));
            qk_exp_frag[1].x[j+2] = qk_exp_frag[1].x[j+10] = exp2f(__half2float(qk_frag[1].x[j+2] - b_max.y));
            qk_exp_frag[1].x[j+3] = qk_exp_frag[1].x[j+11] = exp2f(__half2float(qk_frag[1].x[j+3] - b_max.y));
        }

        #pragma unroll
        for(int j = 0; j < 5; j += 4){
            qk_exp_frag[2].x[j] = qk_exp_frag[2].x[j+8] = exp2f(__half2float(qk_frag[2].x[j] - b_max.x));
            qk_exp_frag[2].x[j+1] = qk_exp_frag[2].x[j+9] = exp2f(__half2float(qk_frag[2].x[j+1] - b_max.x));
            qk_exp_frag[2].x[j+2] = qk_exp_frag[2].x[j+10] = exp2f(__half2float(qk_frag[2].x[j+2] - b_max.y));
            qk_exp_frag[2].x[j+3] = qk_exp_frag[2].x[j+11] = exp2f(__half2float(qk_frag[2].x[j+3] - b_max.y));
        }

        #pragma unroll
        for(int j = 0; j < 5; j += 4){
            qk_exp_frag[3].x[j] = qk_exp_frag[3].x[j+8] = exp2f(__half2float(qk_frag[3].x[j] - b_max.x));
            qk_exp_frag[3].x[j+1] = qk_exp_frag[3].x[j+9] = exp2f(__half2float(qk_frag[3].x[j+1] - b_max.x));
            qk_exp_frag[3].x[j+2] = qk_exp_frag[3].x[j+10] = exp2f(__half2float(qk_frag[3].x[j+2] - b_max.y));
            qk_exp_frag[3].x[j+3] = qk_exp_frag[3].x[j+11] = exp2f(__half2float(qk_frag[3].x[j+3] - b_max.y));
        }

        #pragma unroll
        for(int j = 0; j < 4; j++){
            wmma::load_matrix_sync(v_frag, v_sh + j*16, 72);
            wmma::mma_sync(qkv_frag[j], qk_exp_frag[0], v_frag, qkv_frag[j]);
            wmma::mma_sync(qkv_frag[j+4], qk_exp_frag[1], v_frag, qkv_frag[j+4]);
            wmma::mma_sync(qkv_frag[j+8], qk_exp_frag[2], v_frag, qkv_frag[j+8]);
            wmma::mma_sync(qkv_frag[j+12], qk_exp_frag[3], v_frag, qkv_frag[j+12]);
...
int main(){
    ...
    flash10_async_q_inregs_16x64x32<<<blocks64, 128>>>(dev_Q, dev_K, dev_V, dev_O, s);
    
    flash10_async_q_inregs_32x64x32<<<blocks128, 128>>>(dev_Q, dev_K, dev_V, dev_O, s);

    flash10_async_q_inregs_48x64x32<<<blocks192, 128>>>(dev_Q, dev_K, dev_V, dev_O, s);
    
    flash10_async_q_inregs_64x64x32<<<blocks256, 128>>>(dev_Q, dev_K, dev_V, dev_O, s);
