...
__global__ void flash10_async_q_inregs_16x64x64(half* Q, half* K, half* V, half* O, int s){
    ...
    __shared__ half k_sh[64*72];
    __shared__ half v_sh[64*72];
...
__global__ void flash10_async_q_inregs_32x64x64(half* Q, half* K, half* V, half* O, int s){
    ...
    for(int o = 0; o < s; o += 64){
        for(int i = threadIdx.x; i < 64 * 64 / 8; i += blockDim.x){
            int j = i + i/8;
            // reinterpret_cast<float4*>(k_sh)[j] = reinterpret_cast<float4*>(K)[i];
            // reinterpret_cast<float4*>(v_sh)[j] = reinterpret_cast<float4*>(V)[i];
            uint32_t smem = __cvta_generic_to_shared(reinterpret_cast<float4*>(k_sh)+j);
            CP_ASYNC_CG(smem, reinterpret_cast<float4*>(K) + i, 16);
        ...
        for(int i = 0; i < 4; i ++){
            ...
            for(int j = 0; j < 4; j++){
                wmma::load_matrix_sync(k_frag, k_sh + i*16*72 + j*16, 72);
                wmma::mma_sync(qk_frag[i], q_frag[j], k_frag, qk_frag[i]);
                wmma::mma_sync(qk_frag[i+4], q_frag[j+4], k_frag, qk_frag[i+4]);
            }
        }
        __syncwarp();
        ...
        for(int i = 0; i < 4; i++){
            ...
            b_max.x = max(b_max.x, qk_frag[i+4].x[0]);
            b_max.x = max(b_max.x, qk_frag[i+4].x[1]);
            b_max.y = max(b_max.y, qk_frag[i+4].x[2]);
            b_max.y = max(b_max.y, qk_frag[i+4].x[3]);
            b_max.x = max(b_max.x, qk_frag[i+4].x[4]);
            b_max.x = max(b_max.x, qk_frag[i+4].x[5]);
            b_max.y = max(b_max.y, qk_frag[i+4].x[6]);
            b_max.y = max(b_max.y, qk_frag[i+4].x[7]);
        }
        ...
        for(int i = 0; i < 4; i++){
            ...
            #pragma unroll
            for(int j = 0; j < 5; j += 4){
                qk_exp_frag[1].x[j] = qk_exp_frag[1].x[j+8] = exp2f(__half2float(qk_frag[i+4].x[j] - b_max.x));
                qk_exp_frag[1].x[j+1] = qk_exp_frag[1].x[j+9] = exp2f(__half2float(qk_frag[i+4].x[j+1] - b_max.x));
                qk_exp_frag[1].x[j+2] = qk_exp_frag[1].x[j+10] = exp2f(__half2float(qk_frag[i+4].x[j+2] - b_max.y));
                qk_exp_frag[1].x[j+3] = qk_exp_frag[1].x[j+11] = exp2f(__half2float(qk_frag[i+4].x[j+3] - b_max.y));
            }
...
__global__ void flash10_async_q_inregs_48x64x64(half* Q, half* K, half* V, half* O, int s){
    ...
    wmma::fragment<wmma::accumulator, 16, 16, 16, half> qk_frag[12];
    ...
    for(int o = 0; o < s; o += 64){
        ...
        for(int i = 0; i < 4; i++){
            ...
            wmma::fill_fragment(qk_frag[i+8], 0.f);
            ...
            for(int j = 0; j < 4; j++){
                ...
                wmma::mma_sync(qk_frag[i], q_frag[j], k_frag, qk_frag[i]);
                wmma::mma_sync(qk_frag[i+4], q_frag[j+4], k_frag, qk_frag[i+4]);
                wmma::mma_sync(qk_frag[i+8], q_frag[j+8], k_frag, qk_frag[i+8]);
            }
        }
        ...
        for(int i = 0; i < 4; i++){
            ...
            c_max.x = max(c_max.x, qk_frag[i+8].x[0]);
            c_max.x = max(c_max.x, qk_frag[i+8].x[1]);
            c_max.y = max(c_max.y, qk_frag[i+8].x[2]);
            c_max.y = max(c_max.y, qk_frag[i+8].x[3]);
            c_max.x = max(c_max.x, qk_frag[i+8].x[4]);
            c_max.x = max(c_max.x, qk_frag[i+8].x[5]);
            c_max.y = max(c_max.y, qk_frag[i+8].x[6]);
            c_max.y = max(c_max.y, qk_frag[i+8].x[7]);
        }
        ...
        for(int i = 0; i < 4; i++){
            ...
            #pragma unroll
            for(int j = 0; j < 5; j += 4){
                qk_exp_frag[2].x[j] = qk_exp_frag[2].x[j+8] = exp2f(__half2float(qk_frag[i+8].x[j] - b_max.x));
                qk_exp_frag[2].x[j+1] = qk_exp_frag[2].x[j+9] = exp2f(__half2float(qk_frag[i+8].x[j+1] - b_max.x));
                qk_exp_frag[2].x[j+2] = qk_exp_frag[2].x[j+10] = exp2f(__half2float(qk_frag[i+8].x[j+2] - b_max.y));
                qk_exp_frag[2].x[j+3] = qk_exp_frag[2].x[j+11] = exp2f(__half2float(qk_frag[i+8].x[j+3] - b_max.y));
            }
...
__global__ void flash10_async_q_inregs_64x64x64(half* Q, half* K, half* V, half* O, int s){
    ...
    wmma::fragment<wmma::accumulator, 16, 16, 16, half> qk_frag[16];
    ...
    for(int o = 0; o < s; o += 64){
        ...
        for(int i = 0; i < 4; i++){
            wmma::fill_fragment(qk_frag[i], 0.0f);
            wmma::fill_fragment(qk_frag[i+4], 0.0f);
            wmma::fill_fragment(qk_frag[i+8], 0.0f);
            wmma::fill_fragment(qk_frag[i+12], 0.0f);
            ...
            for(int j = 0; j < 4; j++){
                ...
                wmma::mma_sync(qk_frag[i+4], q_frag[j+4], k_frag, qk_frag[i+4]);
                wmma::mma_sync(qk_frag[i+8], q_frag[j+8], k_frag, qk_frag[i+8]);
                wmma::mma_sync(qk_frag[i+12], q_frag[j+12], k_frag, qk_frag[i+12]);
            }
        }
        ...
        for(int i = 0; i < 4; i++){
            ...
            d_max.x = max(d_max.x, qk_frag[i+12].x[0]);
            d_max.x = max(d_max.x, qk_frag[i+12].x[1]);
            d_max.y = max(d_max.y, qk_frag[i+12].x[2]);
            d_max.y = max(d_max.y, qk_frag[i+12].x[3]);
            d_max.x = max(d_max.x, qk_frag[i+12].x[4]);
            d_max.x = max(d_max.x, qk_frag[i+12].x[5]);
            d_max.y = max(d_max.y, qk_frag[i+12].x[6]);
            d_max.y = max(d_max.y, qk_frag[i+12].x[7]);
        }
        ...
        for(int i = 0; i < 4; i++){
            ...
            #pragma unroll
            for(int j = 0; j < 5; j += 4){
                qk_exp_frag[3].x[j] = qk_exp_frag[3].x[j+8] = exp2f(__half2float(qk_frag[i+12].x[j] - b_max.x));
                qk_exp_frag[3].x[j+1] = qk_exp_frag[3].x[j+9] = exp2f(__half2float(qk_frag[i+12].x[j+1] - b_max.x));
                qk_exp_frag[3].x[j+2] = qk_exp_frag[3].x[j+10] = exp2f(__half2float(qk_frag[i+12].x[j+2] - b_max.y));
                qk_exp_frag[3].x[j+3] = qk_exp_frag[3].x[j+11] = exp2f(__half2float(qk_frag[i+12].x[j+3] - b_max.y));
            }
...
int main(){
    ...
    flash10_async_q_inregs_16x64x64<<<blocks64, 128>>>(dev_Q, dev_K, dev_V, dev_O, s);
    
    flash10_async_q_inregs_32x64x64<<<blocks128, 128>>>(dev_Q, dev_K, dev_V, dev_O, s);

    // flash10_async_q_inregs_48x64x64<<<blocks192, 128>>>(dev_Q, dev_K, dev_V, dev_O, s);
    
    flash10_async_q_inregs_64x64x64<<<blocks256, 128>>>(dev_Q, dev_K, dev_V, dev_O, s);
